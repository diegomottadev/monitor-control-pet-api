...
export const create = async (
  user: { name: string; email: string; roleId: number },
  password: string
): Promise<User> => {
...
export const checkUserRolePermission = async (
  ...
  try {
    ...
    const hasMatchingRole = roles.some(
      (systemRole) => systemRole.name === authenticatedUserRoleName
    );
    ...
    const hasPermission = isAdmin
      ? isAdmin
      : authenticatedUser.role?.permissions?.some(
