...
export const me = (id: number | null = null): Promise<User | null> => {
  ...
  throw new ProfileParameterNotSpecify(
    `Does not specify a parameter ID [${id}] to look up the user profile`
  );
};
...
export const saveUrlImageProfile = async (id: number, imageUrl: string) => {
  try {
    ...
    throw new Error(
      `Error saving urlImageProfile for user with id ${id}: ${error}`
    );
  }
};
